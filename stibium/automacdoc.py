...
def class_name_md(name):
    name_escaped = name.replace("__","\\_\\_")
    return f"## **{name}**`#!py3 class` {{ #{name_escaped} data-toc-label={name_escaped} }}\n\n"

def method_name_md(clas, name, args):
    name_escaped = name.replace("__","\\_\\_")
    return f"### *{clas}*.**{name}**`#!py3 {args}` {{ #{name_escaped} data-toc-label={name_escaped} }}\n\n"

def function_name_md(name, args):
    name_escaped = name.replace("__","\\_\\_")
    return f"## **{name}**`#!py3 {args}` {{ #{name_escaped} data-toc-label={name_escaped} }}\n\n"
...
def write_module(
    ...
    **Parameters**
    ...
    # print(f"package_path: {path_to_home}")
    # print(f"package_path: {package_path}")
    # print(f"module_import: {module_import}")
    a = module_import.split("\\")
    a = ".".join(a)
    print(f"a : {a}")
    module = importlib.import_module(a, package=module_import.split("\\")[0])

    clas = [
        create_class(n, o, ignore_prefix_function)
        for n, o in inspect.getmembers(module, inspect.isclass)
        if inspect.getmodule(o).__name__ in a
    ]
    funs = [
        create_fun(n, o, ignore_prefix_function)
    ...
    md_file = open(path_to_md, "w", encoding="utf-8")
...
def traverse_file_structure(current_node,toc,depth):
    toc+="    "*depth+"- "+current_node.name+":\n"
    if len(current_node.nodes)>0:
        for node in current_node.nodes.keys():
            toc = traverse_file_structure(current_node.get_node(node),toc,depth+1)
    if len(current_node.leafs)>0:
        for leaf in current_node.leafs.values():
            toc+="    "*(depth+1)+"- "+leaf.name+"\n"
    return toc
...
def write_indexmd(path_to_indexmd: str, project_name: str):
    ...
    **Parameters**
    ...
    > **project_name:** `str` -- name of the project
    """
    indexmd_file = open(path_to_indexmd, "w", encoding="utf-8")
...
def write_doc(src, mainfolder):
    ...
    for mod in list_glob:
        ...
        mdfile_long_name = mdfile_path[len(doc_path) + 1 :]
        mdfile_items = mdfile_long_name.split("\\")
        current_node = root
        for item in mdfile_items:
            if item[-3:]!=".md" and current_node.get_node(item)!= None:
                current_node = current_node.get_node(item)
            elif item[-3:]!=".md":
                current_node.add_node(node(item))
                current_node = current_node.get_node(item)
            elif item[-3:]==".md":
                current_node.add_leaf(leaf(mdfile_long_name))

        write_module(root_path, module_name, mdfile_path)

    toc = traverse_file_structure(root,toc,1)
